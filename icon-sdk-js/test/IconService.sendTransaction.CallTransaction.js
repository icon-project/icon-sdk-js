...
const tests = [
  new CallTransactionBuilder()
    ...
    .params({
      ...
      _value: IconService.IconConverter.toHex(IconService.IconAmount.of(1, IconService.IconAmount.Unit.ICX).toLoop()),
    })
    .build(),
  new CallTransactionBuilder()
    .from('hx902ecb51c109183ace539f247b4ea1347fbf23b5')
  ...
  describe('sendTransaction() - CallTransaction', () => {
    tests.forEach((test) => {
      it('should return the tx hash', async () => {
        const txHash = await iconService.sendTransaction(new IconService.SignedTransaction(test, wallet)).execute();
