...
const address = 'hx902ecb51c109183ace539f247b4ea1347fbf23b5';
const privateKey = '38f792b95a5202ab431bfc799f7e1e5c74ec0b9ede5c6142ee7364f2c84d72f6';
const publicKey = 'f87740e3dce5b834b61b6d02f0c3c28dd02ba203cf57fd004eaeb92468d693de76772963c00d255f8371420f3e172444d106847b03b92fc5706a9531f2cf522e';

describe('Wallet', () => {
  describe('loadKeystore()', () => {
    tests.forEach((test) => {
      const wallet = IconService.IconWallet.loadKeystore(test, password, true);

      it('should be same', () => {
        assert.strictEqual(wallet.getAddress(), address);
      });

      it('should be same', () => {
        assert.strictEqual(wallet.getPublicKey(), publicKey);
      });

      it('should be same', () => {
        assert.strictEqual(wallet.getPrivateKey(), privateKey);
      });
    });
  });
});
